...
> **English Title**: Groups REST API Endpoints
>
> 本文档为 Gerrit 用户组，包含完整的 REST API 端点说明、请求/响应示例。
>
> **版本**: v3.8.3
...
- [Find groups that are owned by another group](#find-groups-that-are-owned-by-another-group)
- [Check if a group is owned by the calling user](#check-if-a-group-is-owned-by-the-calling-user)
...
- [Delete Group Description](#delete-group-description)
- [Get Group Options](#get-group-options)
- [Set Group Options](#set-group-options)
- [Get Group Owner](#get-group-owner)
- [Set Group Owner](#set-group-owner)
- [Get Audit Log](#get-audit-log)
- [Index Group](#index-group)
- [Group Member Endpoints](#group-member-endpoints)
...
- [Subgroup Endpoints](#subgroup-endpoints)
- [List Subgroups](#list-subgroups)
- [Get Subgroup](#get-subgroup)
...
- [GroupAuditEventInfo](#groupauditeventinfo)
- [GroupInfo](#groupinfo)
...
This page describes the group related REST endpoints. Please also take note of the general information on the [REST API](rest-api.html).
...
Lists the groups accessible by the caller. This is the same as using the [ls-groups](cmd-ls-groups.html) command over SSH, and accepts the same options as query parameters.
...
By setting `owned-by` and specifying the {group-id} of another group, it is possible to find all the groups for which the owning group is the given group.
...
Queries internal groups visible to the caller. The [query string](user-search-groups.html#_search_operators) must be provided by the `query` parameter. The `start` and `limit` parameters can be used to skip/limit results.
...
The `/groups/?query=<query>` URL also accepts a limit integer in the `limit` parameter. This limits the results to `limit` groups.
...
If the group creation fails because the name is already in use, or the UUID was specified and the UUID is already in use, the response is “409 Conflict”.
...
As result a list of detailed [ AccountInfo](rest-api-accounts.html#account-info) entries is returned. The entries in the list are sorted by full name, preferred email and id.
...
'GET /groups/{group-id}/members/[{account-id}](rest-api-accounts.html#account-id)'
...
As response a detailed [ AccountInfo](rest-api-accounts.html#account-info) entity is returned that describes the group member.
...
'PUT /groups/{group-id}/members/[{account-id}](rest-api-accounts.html#account-id)'
...
'DELETE /groups/{group-id}/members/[{account-id}](rest-api-accounts.html#account-id)'
...
---|---  
`member` | The group member that is added/removed. If `type` is `ADD_USER` or `REMOVE_USER` the member is returned as detailed [AccountInfo](rest-api-accounts.html#account-info) entity, if `type` is `ADD_GROUP` or `REMOVE_GROUP` the member is returned as GroupInfo entity. Note that the `name` in GroupInfo will not be set if the member group is not available.  
...
`user` | The user that did the add/remove as detailed [AccountInfo](rest-api-accounts.html#account-info) entity.  
...
`name` | optional, not set if returned in a map where the group name is used as map key | The name of the group.  
...
`created_on` | only for internal groups | The [timestamp](rest-api.html#timestamp) of when the group was created.  
`_more_groups` | optional, only for internal groups, not set if `false` | Whether the query would deliver more results if not limited.  
Only set on the last group that is returned by a group query.  
`members` | optional, only for internal groups | A list of [AccountInfo](rest-api-accounts.html#account-info) entities describing the direct members.  
Only set if members are requested.  
`includes` | optional, only for internal groups | A list of GroupInfo entities describing the direct subgroups.  
...
`visible_to_all` | optional | Whether the group is visible to all registered users.  
`false` if not set.  
...
[account ids](rest-api-accounts.html#account-id).  
...
Field Name |  | Description  
---|---|---  
`_one_member` | optional | The [id](rest-api-accounts.html#account-id) of one account that should be added or deleted.  
`members` | optional | A list of [account ids](rest-api-accounts.html#account-id) that identify the accounts that should be added or deleted.  
