...
> **English Title**: Config REST API Endpoints
>
> 本文档为 Gerrit 配置，包含完整的 REST API 端点说明、请求/响应示例。
>
> **版本**: v3.8.3
...
- [Config Endpoints (配置端点)](#config-endpoints-(配置端点))
...
- [Flush Several Caches At Once](#flush-several-caches-at-once)
- [Get Cache](#get-cache)
...
- [Get Top Menus](#get-top-menus)
- [Get Default User Preferences](#get-default-user-preferences)
- [Set Default User Preferences](#set-default-user-preferences)
- [Get Default Diff Preferences](#get-default-diff-preferences)
- [Set Default Diff Preferences](#set-default-diff-preferences)
- [Get Default Edit Preferences](#get-default-edit-preferences)
- [Set Default Edit Preferences](#set-default-edit-preferences)
- [Index a set of changes](#index-a-set-of-changes)
- [IDs](#ids)
...
- [AuthInfo](#authinfo)
- [CacheInfo](#cacheinfo)
- [CacheOperationInput](#cacheoperationinput)
- [CapabilityInfo](#capabilityinfo)
- [ChangeConfigInfo](#changeconfiginfo)
- [ChangeIndexConfigInfo](#changeindexconfiginfo)
- [CheckAccountExternalIdsInput](#checkaccountexternalidsinput)
- [CheckAccountExternalIdsResultInfo](#checkaccountexternalidsresultinfo)
- [CheckAccountsInput](#checkaccountsinput)
- [CheckAccountsResultInfo](#checkaccountsresultinfo)
- [CheckGroupsInput](#checkgroupsinput)
- [CheckGroupsResultInfo](#checkgroupsresultinfo)
- [ConsistencyCheckInfo](#consistencycheckinfo)
- [ConsistencyCheckInput](#consistencycheckinput)
- [ConsistencyProblemInfo](#consistencyprobleminfo)
- [ConfigUpdateInfo](#configupdateinfo)
- [ConfigUpdateEntryInfo](#configupdateentryinfo)
- [DownloadInfo](#downloadinfo)
- [DownloadSchemeInfo](#downloadschemeinfo)
- [EmailConfirmationInput](#emailconfirmationinput)
- [EntriesInfo](#entriesinfo)
- [GerritInfo](#gerritinfo)
- [IndexConfigInfo](#indexconfiginfo)
- [HitRatioInfo](#hitratioinfo)
- [IndexChangesInput](#indexchangesinput)
- [JvmSummaryInfo](#jvmsummaryinfo)
- [MemSummaryInfo](#memsummaryinfo)
...
- [ThreadSummaryInfo](#threadsummaryinfo)
- [TopMenuEntryInfo](#topmenuentryinfo)
...
This page describes the config related REST endpoints. Please also take note of the general information on the [REST API](rest-api.html).
...
Not all configuration value can be picked up by this command. Which config sections and values that are supported is documented here: [Configuration](config-gerrit.html)
...
The caller must be a member of a group that is granted one of the following capabilities:
  ...
  * [Administrate Server](access-control.html#capability_administrateServer)
...
The caller must be a member of a group that is granted one of the following capabilities:
  ...
  * [Flush Caches](access-control.html#capability_flushCaches) (any cache except "web_sessions")

  * [Maintain Server](access-control.html#capability_maintainServer) (any cache including "web_sessions")

  * [Administrate Server](access-control.html#capability_administrateServer) (any cache including "web_sessions")
...
The caller must be a member of a group that is granted the [Administrate Server](access-control.html#capability_administrateServer) capability.
...
As response a [ PreferencesInfo](rest-api-accounts.html#preferences-info) is returned.
...
The new user preferences must be provided in the request body as a [PreferencesInput](rest-api-accounts.html#preferences-input) entity.

To be allowed to set default preferences, a user must be a member of a group that is granted the [ Administrate Server](access-control.html#capability_administrateServer) capability.
...
As response a [ DiffPreferencesInfo](rest-api-accounts.html#diff-preferences-info) is returned.
...
The new diff preferences must be provided in the request body as a [ DiffPreferencesInput](rest-api-accounts.html#diff-preferences-input) entity.

To be allowed to set default diff preferences, a user must be a member of a group that is granted the [ Administrate Server](access-control.html#capability_administrateServer) capability.
...
As response a [ EditPreferencesInfo](rest-api-accounts.html#edit-preferences-info) is returned.
...
The new edit preferences must be provided in the request body as a [ EditPreferencesInput](rest-api-accounts.html#edit-preferences-input) entity.

To be allowed to set default edit preferences, a user must be a member of a group that is granted the [ Administrate Server](access-control.html#capability_administrateServer) capability.
...
When `delete_missing` is set to `true` changes to be reindexed which are missing in NoteDb will be deleted in the index.
...
The `AccountsConfigInfo` entity contains information about Gerrit configuration from the [accounts](config-gerrit.html#accounts) section.

Field Name | Description  
---|---  
`visibility` | [Visibility setting for accounts](config-gerrit.html#accounts.visibility).  
`default_display_name` | The default strategy for choosing the display name in the UI, see also [gerrit.config](config-gerrit.html#accounts.defaultDisplayName).  
...
Field Name |  | Description  
---|---|---  
`type` |  | The [authentication type](config-gerrit.html#auth.type) that is configured on the server. Can be `OPENID`, `OPENID_SSO`, `OAUTH`, `HTTP`, `HTTP_LDAP`, `CLIENT_SSL_CERT_LDAP`, `LDAP`, `LDAP_BIND`, `CUSTOM_EXTENSION` or `DEVELOPMENT_BECOME_ANY_ACCOUNT`.  
`use_contributor_agreements` | not set if `false` | Whether [contributor agreements](config-gerrit.html#auth.contributorAgreements) are required.  
`contributor_agreements` | not set if `use_contributor_agreements` is `false` | List of contributor agreements as [ ContributorAgreementInfo](rest-api-accounts.html#contributor-agreement-info) entities.  
...
`login_url` | optional | The [login URL](config-gerrit.html#auth.loginUrl). Only set if [authentication type](config-gerrit.html#auth.type) is `HTTP` or `HTTP_LDAP`.  
`login_text` | optional | The [login text](config-gerrit.html#auth.loginText). Only set if [authentication type](config-gerrit.html#auth.type) is `HTTP` or `HTTP_LDAP`.  
`switch_account_url` | optional | The [URL to switch accounts](config-gerrit.html#auth.switchAccountUrl).  
`register_url` | optional | The [register URL](config-gerrit.html#auth.registerUrl). Only set if [authentication type](config-gerrit.html#auth.type) is `LDAP`, `LDAP_BIND` or `CUSTOM_EXTENSION`.  
`register_text` | optional | The [register text](config-gerrit.html#auth.registerText). Only set if [authentication type](config-gerrit.html#auth.type) is `LDAP`, `LDAP_BIND` or `CUSTOM_EXTENSION`.  
`edit_full_name_url` | optional | The [URL to edit the full name](config-gerrit.html#auth.editFullNameUrl). Only set if [authentication type](config-gerrit.html#auth.type) is `LDAP`, `LDAP_BIND` or `CUSTOM_EXTENSION`.  
`http_password_url` | optional | The [URL to obtain an HTTP password](config-gerrit.html#auth.httpPasswordUrl). Only set if [authentication type](config-gerrit.html#auth.type) is `CUSTOM_EXTENSION`.  
`git_basic_auth_policy` | optional | The [policy](config-gerrit.html#auth.gitBasicAuthPolicy) to authenticate Git over HTTP and REST API requests when [authentication type](config-gerrit.html#auth.type) is `LDAP`, `LDAP_BIND` or `OAUTH`. Can be `HTTP`, `LDAP`, `HTTP_LDAP` or `OAUTH`.  
...
`caches` | optional | A list of cache names. This list defines the caches on which the specified operation should be executed. Whether this list must be specified depends on the operation being executed.  
...
The `ChangeConfigInfo` entity contains information about Gerrit configuration from the [change](config-gerrit.html#change) section.

Field Name |  | Description  
---|---|---  
`allow_blame` | not set if `false` | [Whether blame on side by side diff is allowed](config-gerrit.html#change.allowBlame).  
`update_delay` |  | [How often in seconds the web interface should poll for updates to the currently open change](config-gerrit.html#change.updateDelay).  
`submit_whole_topic` | not set if `false` | [A configuration if the whole topic is submitted](config-gerrit.html#change.submitWholeTopic).  
`disable_private_changes` | not set if `false` | Returns true if private changes are disabled.  
`mergeability_computation_behavior` |  | Value of the [ configuration parameter](config-gerrit.html#change.mergeabilityComputationBehavior) that controls whether the mergeability bit in [ChangeInfo](rest-api-changes.html#change-info) will never be set and if the bit is indexed.  
`enable_robot_comments` | not set if `false` | [Are robot comments enabled?](config-gerrit.html#change.enableRobotComments).  
`conflicts_predicate_enabled` | not set if `false` | [Are conflicts enabled?](config-gerrit.html#change.conflictsPredicateEnabled).  
  
### ChangeIndexConfigInfo

The `ChangeIndexConfigInfo` entity contains information about Gerrit configuration from the [index.change](config-gerrit.html#index.change) section.

Field Name |  | Description  
---|---|---  
`index_mergeable` | not set if `false` | Value of the [ configuration parameter](config-gerrit.html#index.change.indexMergeable) that controls whether the mergeability bit is indexed (hence queryable using `is:mergeable`).  
  
### CheckAccountExternalIdsInput

The `CheckAccountExternalIdsInput` entity contains input for the account external ID consistency check.

Currently this entity contains no fields.

### CheckAccountExternalIdsResultInfo

The `CheckAccountExternalIdsResultInfo` entity contains the result of running the account external ID consistency check.
...
If a changed config value is missing from the `applied` and the `rejected` lists there are no guarantees to whether they have or have not taken effect.

Field Name | Description  
---|---  
`applied` | A list of ConfigUpdateEntryInfos describing the applied configuration changes.  
Every config value change representation present in this list is guaranteed to have taken effect.  
`rejected` | A list of ConfigUpdateEntryInfos describing the rejected configuration changes.   
Every config value change representation present in this list is guaranteed not to have taken effect.  
...
The `GerritInfo` entity contains information about Gerrit configuration from the [gerrit](config-gerrit.html#gerrit) section.

Field Name |  | Description  
---|---|---  
`all_projects_name` |  | Name of the [root project](config-gerrit.html#gerrit.allProjects).  
`all_users_name` |  | Name of the [project in which meta data of all users is stored](config-gerrit.html#gerrit.allUsers).  
...
`report_bug_url` | optional | [URL to report bugs](config-gerrit.html#gerrit.reportBugUrl).  
`instance_id` | optional | [Short identifier for this Gerrit installation](config-gerrit.html#gerrit.instanceId).  
  
### IndexConfigInfo

The `IndexConfigInfo` entity contains information about Gerrit configuration from the [index](config-gerrit.html#index) section.

Field Name |  | Description  
---|---|---  
`change` |  | Information about the configuration from the [index.change](config-gerrit.html#index.change) section as ChangeIndexConfigInfo entity.  
...
Field Name |  | Description  
---|---|---  
`changes` |  | List of [change-ids](rest-api-changes.html#change-id)  
`delete_missing` | optional | Delete changes which are missing in NoteDb from the index. This can be used to get rid of stale index entries. Possible values are `true` and `false`. By default set to `false`.  
...
Field Name |  | Description  
---|---|---  
`enableSignedPush` | optional | Whether signed push validation support is enabled on the server; see the [global configuration](config-gerrit.html#receive.certNonceSeed) for details.  
...
Field Name |  | Description  
---|---|---  
`accounts` |  | Information about the configuration from the [accounts](config-gerrit.html#accounts) section as AccountsConfigInfo entity.  
`auth` |  | Information about the authentication configuration as AuthInfo entity.  
`change` |  | Information about the configuration from the [change](config-gerrit.html#change) section as ChangeConfigInfo entity.  
`download` |  | Information about the configured download options as DownloadInfo entity. information about Gerrit  
`gerrit` |  | Information about the configuration from the [gerrit](config-gerrit.html#gerrit) section as  GerritInfo entity.  
`index` |  | Information about the configuration from the [index](config-gerrit.html#index) section as  IndexConfigInfo entity.  
...
`sshd` | optional | Information about the configuration from the [sshd](config-gerrit.html#sshd) section as SshdInfo entity. Not set if SSHD is disabled.  
`suggest` |  | Information about the configuration from the [suggest](config-gerrit.html#suggest) section as  SuggestInfo entity.  
`user` |  | Information about the configuration from the [user](config-gerrit.html#user) section as  UserConfigInfo entity.  
`default_theme` | optional | URL to a default Gerrit UI theme plugin, if available. Located in `/static/gerrit-theme.js` by default.  
`submit_requirement_dashboard_columns` |  | The list of submit requirement names that should be displayed as separate columns in the dashboard. If empty, the default is to display all submit requirements that are applicable for changes appearing in the dashboard.  
  
### SshdInfo

The `SshdInfo` entity contains information about Gerrit configuration from the [sshd](config-gerrit.html#sshd) section.
...
The `SuggestInfo` entity contains information about Gerrit configuration from the [suggest](config-gerrit.html#suggest) section.

Field Name | Description  
---|---  
`from` | The [number of characters](config-gerrit.html#suggest.from) that a user must have typed before suggestions are provided.  
...
The `UserConfigInfo` entity contains information about Gerrit configuration from the [user](config-gerrit.html#user) section.

Field Name | Description  
---|---  
`anonymous_coward_name` | [Username](config-gerrit.html#user.anonymousCoward) that is displayed in the Gerrit Web UI and in e-mail notifications if the full name of the user is not set.  
