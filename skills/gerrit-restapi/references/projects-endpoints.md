...
> **English Title**: Projects REST API Endpoints
>
> 本文档为 Gerrit 项目，包含完整的 REST API 端点说明、请求/响应示例。
>
> **版本**: v3.8.3
...
- [Delete Project Description](#delete-project-description)
- [Get Project Parent](#get-project-parent)
...
- [Ban Commit](#ban-commit)
- [List Access Rights for Project](#list-access-rights-for-project)
- [Add, Update and Delete Access Rights for Project](#add,-update-and-delete-access-rights-for-project)
- [Create Change for review.](#create-change-for-review)
- [Create Access Rights Change for review.](#create-access-rights-change-for-review)
- [Check Access](#check-access)
- [Check Access Options](#check-access-options)
- [Index project](#index-project)
- [Index all changes in a project](#index-all-changes-in-a-project)
- [Check project consistency](#check-project-consistency)
- [Get Commits Included In Refs](#get-commits-included-in-refs)
...
- [Get Mergeable Information](#get-mergeable-information)
- [Get Reflog](#get-reflog)
- [Child Project Endpoints](#child-project-endpoints)
- [List Child Projects](#list-child-projects)
- [Get Child Project](#get-child-project)
...
- [Batch Update Labels](#batch-update-labels)
- [Submit Requirement Endpoints](#submit-requirement-endpoints)
- [Create Submit Requirement](#create-submit-requirement)
- [Update Submit Requirement](#update-submit-requirement)
- [Get Submit Requirement](#get-submit-requirement)
- [List Submit Requirements](#list-submit-requirements)
- [Delete Submit Requirement](#delete-submit-requirement)
- [IDs](#ids)
- [{branch-id}](#{branch-id})
- [{commit-id}](#{commit-id})
- [{tag-id}](#{tag-id})
...
- [{submit-requirement-name}](#{submit-requirement-name})
- [{project-name}](#{project-name})
- [JSON Entities](#json-entities)
- [AccessCheckInfo](#accesscheckinfo)
- [AutoCloseableChangesCheckInput](#autocloseablechangescheckinput)
- [AutoCloseableChangesCheckResult](#autocloseablechangescheckresult)
- [BanInput](#baninput)
- [BanResultInfo](#banresultinfo)
- [BranchInfo](#branchinfo)
- [BranchInput](#branchinput)
- [CheckProjectInput](#checkprojectinput)
- [CheckProjectResultInfo](#checkprojectresultinfo)
- [CommentLinkInfo](#commentlinkinfo)
- [CommentLinkInput](#commentlinkinput)
- [ConfigInfo](#configinfo)
- [ConfigInput](#configinput)
- [ConfigParameterInfo](#configparameterinfo)
- [DashboardInfo](#dashboardinfo)
- [DashboardInput](#dashboardinput)
- [DashboardSectionInfo](#dashboardsectioninfo)
...
- [LabelDefinitionInfo](#labeldefinitioninfo)
- [LabelDefinitionInput](#labeldefinitioninput)
- [LabelTypeInfo](#labeltypeinfo)
- [MaxObjectSizeLimitInfo](#maxobjectsizelimitinfo)
...
- [ProjectParentInput](#projectparentinput)
- [ReflogEntryInfo](#reflogentryinfo)
...
This page describes the project related REST endpoints. Please also take note of the general information on the [REST API](rest-api.html).
...
Lists the projects accessible by the caller. This is the same as using the [ls-projects](cmd-ls-projects.html) command over SSH, and accepts the same options as query parameters.
...
Note |  If the calling user does not meet any of the following criteria:   
---|---  
...
Queries projects visible to the caller. The [query string](user-search-projects.html#_search_operators) must be provided by the `query` parameter. The `start` and `limit` parameters can be used to skip/limit results.
...
A commit message can be provided in the request body as a ProjectDescriptionInput entity. Historically, this method allowed a body in the DELETE, but that behavior is [deprecated](https://www.gerritcodereview.com/releases/2.16.md). In this case, use PUT instead.
...
The `Location` header of the response refers to the [background task](rest-api-config.html#get-task) which allows to inspect the progress of its execution. In case of asynchronous execution the `show_progress` option is ignored.
...
Marks commits as banned for the project. If a commit is banned Gerrit rejects every push that includes this commit with [contains banned commit …​](error-contains-banned-commit.html).

Note |  This REST endpoint only marks the commits as banned, but it does not remove the commits from the history of any central branch. This needs to be done manually.   
---|---  
...
'GET /projects/[{project-name}](rest-api-projects.html#project-name)/access'

Lists the access rights for a single project.

As result a [ProjectAccessInfo](rest-api-access.html#project-access-info) entity is returned.
...
'POST /projects/[{project-name}](rest-api-projects.html#project-name)/access'
...
This endpoint is functionally equivalent to [create change in the change API](rest-api-changes.html#create-change), but it has the project name in the URL, which is easier to route in sharded deployments.
...
'PUT /projects/[{project-name}](rest-api-projects.html#project-name)/access:review
...
This command runs access checks for other users. This requires the [View Access](access-control.html#capability_viewAccess) global capability.
...
This REST endpoint requires the [Administrate Server](access-control.html#capability_administrateServer) global capability.
...
'GET /projects/{project-name}/branches/{branch-id}/files/[{file-id}](rest-api-changes.html#file-id)/content'
...
The `source` query parameter is required, which can be anything that could be resolved to a commit, and is visible to the caller. See examples of the `source` attribute in [MergeInput](rest-api-changes.html#merge-input).
...
As response a [MergeableInfo](rest-api-changes.html#mergeable-info) entity is returned.
...
As response a [CommitInfo](rest-api-changes.html#commit-info) entity is returned that describes the commit.
...
Retrieves the branches and tags in which a change is included. As result an [IncludedInInfo](rest-api-changes.html#included-in-info) entity is returned.
...
'GET /projects/{project-name}/commits/{commit-id}/files/[{file-id}](rest-api-changes.html#file-id)/content'
...
To cherry pick a change revision, see [CherryPick](rest-api-changes.html#cherry-pick).

The destination branch must be provided in the request body inside a [CherryPickInput](rest-api-changes.html#cherrypick-input) entity. If the commit message is not set, the commit message of the source commit will be used.
...
As response a [ChangeInfo](rest-api-changes.html#change-info) entity is returned that describes the resulting cherry-picked change.
...
As result a map is returned that maps the [file path](rest-api-changes.html#file-id) to a [FileInfo](rest-api-changes.html#file-info) entry. The entries in the map are sorted by file path.
...
If a submit requirement with this name is already defined in this project, this submit requirement definition is updated (see Update Submit Requirement).

The submit requirement data must be provided in the request body as SubmitRequirementInput.
...
As response a SubmitRequirementInfo entity is returned that describes the created submit requirement.
...
The new submit requirement will overwrite the existing submit requirement. That is, unspecified attributes will be set to their defaults.
...
'GET /projects/{project-name}/submit_requirements/{submit-requirement-name}'
...
'GET /projects/{project-name}/submit_requirements'
...
The calling user must have read access to the `refs/meta/config` branch of the project. If the `inherited` option is used, the caller must have read access to the `refs/meta/config` branch of all parent projects as well.

As response a list of SubmitRequirementInfo entities is returned.
...
'DELETE /projects/{project-name}/submit_requirements/{submit-requirement-name}'

Deletes the definition of a submit requirement that is defined in this project.

The calling user must have write access to the `refs/meta/config` branch of the project. The request would fail with `404 Not Found` if there is no submit requirement with this name for this project.
...
`debug_logs` | optional | Debug logs that may help to understand why a permission is denied or allowed.  
...
Field Name | Description  
---|---  
`auto_closeable_changes` | Changes that can be auto-closed as list of [ChangeInfo](rest-api-changes.html#change-info) entities. For each returned [ChangeInfo](rest-api-changes.html#change-info) entity the `problems` field is populated that includes details about the detected issues. If `fix` in the  AutoCloseableChangesCheckInput was set to `true`, `status` and `outcome` in [ProblemInfo](rest-api-changes.html#problem-info) are populated. If the status says `FIXED` Gerrit was able to auto-close the change now.  
...
`web_links` | optional | Links to the branch in external sites as a list of [WebLinkInfo](rest-api-changes.html#web-link-info) entries.  
...
Field Name |  | Description  
---|---|---  
`auto_closeable_changes_check` | optional | Parameters for the  AutoCloseableChangesCheck as [ AutoCloseableChangesCheckInput](rest-api-changes.html#auto_closeable_changes_check_input) entity.  
...
Field Name |  | Description  
---|---|---  
`auto_closeable_changes_check_result` | optional | Results for the  AutoCloseableChangesCheck as [ AutoCloseableChangesCheckResult](rest-api-changes.html#auto_closeable_changes_check_result) entity.  
  
### CommentLinkInfo

The `CommentLinkInfo` entity describes a [commentlink](config-gerrit.html#commentlink).

Field Name |  | Description  
---|---|---  
`match` |  | A JavaScript regular expression to match positions to be replaced with a hyperlink, as documented in [commentlink.name.match](config-gerrit.html#commentlink.name.match).  
`link` |  | The URL to direct the user to whenever the regular expression is matched, as documented in [commentlink.name.link](config-gerrit.html#commentlink.name.link).  
`prefix` | optional | Text inserted before the link.  
`suffix` | optional | Text inserted after the link.  
`text` | optional | Text of the link.  
`enabled` | optional | Whether the commentlink is enabled, as documented in [ commentlink.name.enabled](config-gerrit.html#commentlink.name.enabled). If not set the commentlink is enabled.  
  
### CommentLinkInput

The `CommentLinkInput` entity describes the input for a [commentlink](config-gerrit.html#commentlink).
...
`require_change_id` | optional | InheritedBooleanInfo that tells whether a valid [Change-Id](user-changeid.html) footer in any commit uploaded for review is required. This does not apply to commits pushed directly to a branch or tag. This property is deprecated and will be removed in a future release.  
...
`max_object_size_limit` |  | The [max object size limit](config-gerrit.html#receive.maxObjectSizeLimit) of this project as a  MaxObjectSizeLimitInfo entity.  
...
`state` | optional | The state of the project, can be `ACTIVE`, `READ_ONLY` or `HIDDEN`.  
...
`reject_empty_commit` | optional | InheritedBooleanInfo that tells whether empty commits should be rejected when a change is merged. [ActionInfo](rest-api-changes.html#action-info) entities.  
`skip_adding_author_and_committer_as_reviewers` | optional | Whether to skip adding the Git commit author and committer as reviewers for a new change.  
...
`use_contributor_agreements` | optional | Whether authors must complete a contributor agreement on the site before pushing any commits or changes to this project.  
Can be `TRUE`, `FALSE` or `INHERIT`.  
If not set, this setting is not updated.  
`use_content_merge` | optional | Whether Gerrit will try to perform a 3-way merge of text file content when a file has been modified by both the destination branch and the change being submitted. This option only takes effect if submit type is not FAST_FORWARD_ONLY.  
Can be `TRUE`, `FALSE` or `INHERIT`.  
If not set, this setting is not updated.  
`use_signed_off_by` | optional | Whether each change must contain a Signed-off-by line from either the author or the uploader in the commit message.  
Can be `TRUE`, `FALSE` or `INHERIT`.  
If not set, this setting is not updated.  
`create_new_change_for_all_not_in_target` | optional | Whether a new change will be created for every commit not in target branch.  
Can be `TRUE`, `FALSE` or `INHERIT`.  
If not set, this setting is not updated.  
`require_change_id` | optional | Whether a valid [Change-Id](user-changeid.html) footer in any commit uploaded for review is required. This does not apply to commits pushed directly to a branch or tag.  
Can be `TRUE`, `FALSE` or `INHERIT`.  
If not set, this setting is not updated. This property is deprecated and will be removed in a future release.  
`reject_implicit_merges` | optional | Whether a check for implicit merges will be performed when changes are pushed for review.  
Can be `TRUE`, `FALSE` or `INHERIT`.  
If not set, this setting is not updated.  
`max_object_size_limit` | optional | The [max object size limit](config-gerrit.html#receive.maxObjectSizeLimit) of this project as a  MaxObjectSizeLimitInfo entity.  
If set to `0`, the max object size limit is removed.  
If not set, this setting is not updated.  
`submit_type` | optional | The default submit type of the project, can be `MERGE_IF_NECESSARY`, `FAST_FORWARD_ONLY`, `REBASE_IF_NECESSARY`, `REBASE_ALWAYS`, `MERGE_ALWAYS` or `CHERRY_PICK`.  
...
`reject_empty_commit` | optional | Whether empty commits should be rejected when a change is merged. Can be `TRUE`, `FALSE` or `INHERIT`.  
If not set, this setting is not updated.  
...
`foreach` | optional | Subquery that applies to all sections in the dashboard.  
Tokens such as `${project}` are not resolved.  
`url` |  | The URL under which the dashboard can be opened in the Gerrit Web UI.  
...
`configured_value` |  | The configured value, can be `TRUE`, `FALSE` or `INHERIT`.  
`inherited_value` | optional | The boolean value inherited from the parent.  
Not set if there is no parent.  
  
### LabelDefinitionInfo

The `LabelDefinitionInfo` entity describes a [ review label](config-labels.html).

Field Name |  | Description  
---|---|---  
`name` |  | The [name](config-labels.html#label_name) of the label.  
`description` | optional | The description of the label.  
`project_name` |  | The name of the project in which this label is defined.  
`function` |  | The [function](config-labels.html#label_function) of the label (can be `MaxWithBlock`, `AnyWithBlock`, `MaxNoBlock`, `NoBlock`, `NoOp` and `PatchSetLock`.  
`values` |  | The [values](config-labels.html#label_value) of the label as a map of label value to value description. The label values are formatted strings, e.g. "+1" instead of "1", " 0" instead of "0".  
`default_value` |  | The [default value](config-labels.html#label_defaultValue) of the label (as integer).  
`branches` | optional | A list of [branches](config-labels.html#label_branch) for which the label applies. A branch can be a ref, a ref pattern or a regular expression. If not set, the label applies for all branches.  
`can_override` | `false` if not set | Whether this label can be [overridden](config-labels.html#label_canOverride) by child projects.  
`copy_condition` | optional | See [copyCondition](config-labels.html#label_copyCondition).  
`allow_post_submit` | `false` if not set | Whether [allowPostSubmit](config-labels.html#label_allowPostSubmit) is set on the label.  
`ignore_self_approval` | `false` if not set | Whether [ignoreSelfApproval](config-labels.html#label_ignoreSelfApproval) is set on the label.  
  
### LabelDefinitionInput

The `LabelDefinitionInput` entity describes a [ review label](config-labels.html).
...
`name` | optional | The new [name](config-labels.html#label_name) of the label.+ For label creation the name is required if this `LabelDefinitionInput` entity is contained in a BatchLabelInput entity.  
`description` | optional | The new description for the label.  
`function` | optional | The new [function](config-labels.html#label_function) of the label (can be `MaxWithBlock`, `AnyWithBlock`, `MaxNoBlock`, `NoBlock`, `NoOp` and `PatchSetLock`.  
`values` | optional | The new [values](config-labels.html#label_value) of the label as a map of label value to value description. The label values are formatted strings, e.g. "+1" instead of "1", " 0" instead of "0".  
`default_value` | optional | The new [default value](config-labels.html#label_defaultValue) of the label (as integer).  
`branches` | optional | The new branches for which the label applies as a list of [branches](config-labels.html#label_branch). A branch can be a ref, a ref pattern or a regular expression. If not set, the label applies for all branches.  
`can_override` | optional | Whether this label can be [overridden](config-labels.html#label_canOverride) by child projects.  
`copy_condition` | optional | See [copyCondition](config-labels.html#label_copyCondition).  
`unset_copy_condition` | optional | If true, clears the value stored in `copy_condition`.  
`allow_post_submit` | optional | Whether [allowPostSubmit](config-labels.html#label_allowPostSubmit) is set on the label.  
`ignore_self_approval` | optional | Whether [ignoreSelfApproval](config-labels.html#label_ignoreSelfApproval) is set on the label.  
...
The `MaxObjectSizeLimitInfo` entity contains information about the [max object size limit](config-gerrit.html#receive.maxObjectSizeLimit) of a project.

Field Name |  | Description  
---|---|---  
`value` | optional | The effective value in bytes of the max object size limit.  
Not set if there is no limit for the object size.  
`configured_value` | optional | The max object size limit that is configured on the project as a formatted string.  
Not set if there is no limit for the object size configured on project level.  
`summary` | optional | A string describing whether the value was inherited or overridden from the parent project or global config.  
...
`create` | optional | List of LabelDefinitionInput entities that describe labels that should be created.  
`update` | optional | Map of label names to LabelDefinitionInput entities that describe the updates that should be done for the labels.  
...
Field Name |  | Description  
---|---|---  
`remove` | optional | A list of deductions to be applied to the project access as [ProjectAccessInfo](rest-api-access.html#project-access-info) entities.  
`add` | optional | A list of additions to be applied to the project access as [ProjectAccessInfo](rest-api-access.html#project-access-info) entities.  
...
`parent` | optional | The name of the parent project.  
...
`web_links` | optional | Links to the project in external sites as a list of [WebLinkInfo](rest-api-changes.html#web-link-info) entries.  
...
Field Name |  | Description  
---|---|---  
`name` | optional | The name of the project (not encoded).  
...
`submit_type` | optional | The submit type that should be set for the project (`MERGE_IF_NECESSARY`, `REBASE_IF_NECESSARY`, `REBASE_ALWAYS`, `FAST_FORWARD_ONLY`, `MERGE_ALWAYS`, `CHERRY_PICK`).  
If not set, `MERGE_IF_NECESSARY` is set as submit type unless [ repository.<name>.defaultSubmitType](config-gerrit.html#repository.name.defaultSubmitType) is set to a different value.  
`branches` | optional | A list of branches that should be initially created.  
For the branch names the `refs/heads/` prefix can be omitted.  
The first entry of the list will be the [default branch](project-configuration.html#default-branch).  
If the list is empty, [host-level default](config-gerrit.html#gerrit.defaultBranch) is used.  
Branches in the Gerrit internal ref space are not allowed, such as refs/groups/, refs/changes/, etc…​  
`owners` | optional | A list of groups that should be assigned as project owner.  
Each group in the list must be specified as [group-id](rest-api-groups.html#group-id).  
If not set, the [ groups that are configured as default owners](config-gerrit.html#repository.name.ownerGroup) are set as project owners.  
...
`use_content_merge` | `INHERIT` if not set | Whether content merge should be enabled for the project (`TRUE`, `FALSE`, `INHERIT`).  
...
`who` | The user performing the change as a [GitPersonInfo](rest-api-changes.html#git-person-info) entity.  
...
`allow_override_in_child_projects` |  | Whether this submit requirement can be overridden in child projects.  
...
`submittability_expression` |  | Query expression that can be evaluated on any change. If evaluated to true on a change, the submit requirement is fulfilled and not blocking change submission.  
`override_expression` | optional | Query expression that can be evaluated on any change. If evaluated to true on a change, the submit requirement is overridden and not blocking change submission.  
`allow_override_in_child_projects` | optional | Whether this submit requirement can be overridden in child projects. Default is `false`.  
  
### SubmitTypeInfo

Information about the [default submit type of a project](config-project-config.html#submit-type), taking into account project inheritance.
...
`tagger` | Only set for annotated tags, if present in the tag. | The tagger as a [GitPersonInfo](rest-api-changes.html#git-person-info) entity.  
`created` | optional | The [timestamp](rest-api.html#timestamp) of when the tag was created. For annotated and signed tags, this is the timestamp of the tag object and is the same as the `date` field in the `tagger`. For lightweight tags, it is the commit timestamp of the commit to which the tag points, when the object is a commit. It is not set when the object is any other type.  
`can_delete` | not set if `false` | Whether the calling user can delete this tag.  
`web_links` | optional | Links to the tag in external sites as a list of [WebLinkInfo](rest-api-changes.html#web-link-info) entries.  
