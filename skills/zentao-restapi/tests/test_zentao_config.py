...
def _write_config(tmp_path: Path, data: dict, name: str = "zentao.json") -> Path:
    bicv = tmp_path / ".bicv"
...
class TestLoadSystemsConfig:
    ...
    def test_missing_file_raises(self, tmp_path, monkeypatch):
        ...
        with pytest.raises(ServiceError, match="Cannot find config file"):
            sc.load_systems_config("zentao.json")

    def test_invalid_json_raises(self, tmp_path, monkeypatch):
        bicv = tmp_path / ".bicv"
        bicv.mkdir()
        (bicv / "zentao.json").write_text("{not json", encoding="utf-8")
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
class TestSystemMatches:
    ...
    def test_hostname_match(self):
        sc_conf = {"url": "https://zentao.example.com/r"}
        assert sc.system_matches("zentao.example.com", "default", sc_conf) is True

    def test_url_match(self):
        sc_conf = {"url": "https://zentao.example.com"}
        assert sc.system_matches("https://zentao.example.com", "default", sc_conf) is True
...
class TestFindSystem:
    ...
    def test_multiple_matches_raises(self):
        systems = {
            "a": {"url": "https://zentao.example.com"},
            "b": {"url": "https://zentao.example.com"},
        }
        with pytest.raises(ServiceError, match="matches multiple configs"):
            sc.find_system("zentao.example.com", systems, "/path")
...
class TestResolveTarget:
    def test_resolve_from_config_default_system(self, tmp_path, monkeypatch):
        home = _write_config(tmp_path, _full_config())
        monkeypatch.setattr(Path, "home", lambda: home)
        t = sc.resolve_target(None, None, None, config_name="zentao.json", password_key="password")
        assert t.url == "https://zentao.example.com"
    ...
    def test_explicit_system_overrides_default(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: home)
        t = sc.resolve_target(
            None, None, "other", config_name="zentao.json", password_key="password"
        )
    ...
    def test_user_override_takes_precedence(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: home)
        t = sc.resolve_target(
            None, "cliuser:clipass", None, config_name="zentao.json", password_key="password"
        )
    ...
    def test_no_system_and_no_default_raises(self, tmp_path, monkeypatch):
        ...
        with pytest.raises(ServiceError, match="No --system specified"):
            sc.resolve_target(None, None, None, config_name="zentao.json")
    ...
    def test_unknown_system_raises(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: home)
        with pytest.raises(ServiceError, match="does not exist"):
            sc.resolve_target(None, None, "ghost", config_name="zentao.json")
