...
description: |
  ...
  流程分 collect → fetch → analyze → report 四阶段，每阶段落盘中间产物，可基于产物单独重跑、方便
  调试。本 skill 不含脚本，全程由 agent 编排，依赖 gerrit-restapi skill 的 gerrit_api.py 子命令
  完成实际 Gerrit 调用。当用户要「看近期团队入库了哪些代码」「统计某段时间内 merged 的 change」
  「分析入库代码量、review 效率，或 +2 是否抢在 AI 评审之前」时使用。
---

# Gerrit 入库代码每日分析

本技能扫描配置的用户列表在指定时间段内**提交并入库（status:merged）**的 change，按人统计数量、
代码量、效率指标，**检查 AI 评审前置合规**，并产出明细清单。

## 核心约定

- **分析对象**：`~/.bicv/gerrit_daily_analysis.json` 里 `users` 列出的每个用户。每个 user 的值是
  **Gerrit 账号邮箱**，直接作为 Gerrit `owner:` 谓词（如 `owner:lizhe@bicv.com`）。
- **入库定义**：`status:merged`（已合并）。不统计 open / abandoned。
- **时间窗口**：**用户在 prompt 里给时间段**（如「上周」「6/1~6/25」「最近 3 天」），agent 解析成
  Gerrit `after:"..."` / `before:"..."` 谓词；**用户没给则默认滚动 24 小时**（now-24h 到 now）。
...
- **AI 评审前置合规**：每个 merged change 若出现 Code-Review **+2**，则该 +2 的时间点必须**晚于**
  AI 评审结论的时间点，否则算违规（详见 analyze 阶段）。AI 评审通过 message 文本关键词或 label
  投票 `tag` 识别（见配置）。
- **依赖**：本 skill 不含脚本，所有 Gerrit 调用通过 gerrit-restapi skill 的
  `scripts/gerrit_api.py` 子命令完成。使用前确认 gerrit-restapi skill 已安装。
...
落盘路径不写死，统一由 `~/.bicv/common.json` 配置（与 jenkins_daily_analysis 共用同一份）：
...
- `output_root` 缺省 `~/.bicv/output`；`skills.gerrit_daily_analysis` 缺省
  `gerrit_daily_analysis`。
- 每次运行落到 `<output_root>/<skills.gerrit_daily_analysis>/<本地时间戳>/`，时间戳格式
  `YYYY-MM-DD_HHMMSS`（本地时间，Python `datetime.now().strftime`）。多次运行互不覆盖，方便回看
  历史。
...
2. 对每个 change 调（**分批并行**）：
   ...
   ```
   写入 `details/<_number>.json`。⚠️ `get-change` 输出含 `System:` / `变更详情:` 前导行（走
...
> 不再拉文件清单：文件 diff 不纳入本报告，代码量用 collect 已拿到的 change 级
> `insertions` / `deletions` 汇总值。
>
...
用户不指定阶段时，默认**一气呵成**跑 collect → fetch → analyze → report。用户显式说「只跑某阶段」
时，基于运行目录里已有产物重跑该阶段（需用户指明或由 agent 选取最近一次运行目录）。

## 前置检查

1. 确认 gerrit-restapi skill 已安装（`gerrit_api.py` 可用，且已支持 `query-changes --json
   --option`）。未安装或版本过旧则提示用户先装/更新。
2. 确认 `~/.bicv/gerrit.json` 存在且配置了目标 Gerrit（复用 gerrit-restapi 的配置引导）。多实例
   用 `--system <name>` 切换，本 skill 透传。
3. 确认 `~/.bicv/gerrit_daily_analysis.json` 存在且 `users` 非空。不存在则用 AskUserQuestion
   收集 users 列表后创建。`ai_review` 缺省时按默认关键词 / vote_tag 工作。
4. 确认/创建 `~/.bicv/common.json`（见「输出位置」）。

## 禁止

- 不绕过 gerrit-restapi skill 直接发 HTTP 请求；所有 Gerrit 调用走 `gerrit_api.py`。
- 不在本 skill 内复制 gerrit 调用代码。
- 不在脚本里硬编码 LLM 调用——指标计算与合规判定由 agent 自身能力完成。
