...
> ⚠️ **从旧版本升级？** 配置文件已从 `~/.bicv/database.json` 重命名为 `~/.bicv/mysql.json`。迁移: `mv ~/.bicv/database.json ~/.bicv/mysql.json`。

## 首次使用：配置引导

首次使用本技能时，先检查配置文件 `~/.bicv/mysql.json` 是否存在。

**若配置不存在**，使用 AskUserQuestion 引导用户完成配置，生成 `~/.bicv/mysql.json`：
...
提供 MySQL 数据库的 INSERT、UPDATE、SELECT 操作能力。支持通过 `~/.bicv/mysql.json` 配置多个 MySQL 服务器实例，通过 `--system` 参数在实例间切换。所有凭证不硬编码。
...
配置文件 `~/.bicv/mysql.json` 的字段说明：`references/config-schema.md`
