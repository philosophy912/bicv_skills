...
**完整读取**
- `bug_daily_analysis/`：bug_analysis.py、render_charts.py、render_report.py
- `mysql/scripts/mysql_query.py`
- `jenkins_daily_analysis/scripts/collect.py`
...
- **位置**：`skills/jenkins_daily_analysis/scripts/collect.py:92` + `skills/jenkins-restapi/scripts/jenkins_api.py:238,457`
...
- **位置**：`skills/jenkins_daily_analysis/scripts/report.py:277,159`
...
- **位置**：`skills/bug_daily_analysis/scripts/bug_analysis.py:245-246, 283-284, 351, 398`
...
- **位置**：`skills/bug_daily_analysis/scripts/render_report.py:205,168,173`
...
- **位置**：`skills/bug_daily_analysis/scripts/bug_analysis.py:349`（Zentao）vs `:396`（Redmine）
...
- **位置**：`skills/jenkins_daily_analysis/scripts/fetch.py:43,94`
...
- **验证**：`python3 -m pytest` 全量 **945 passed**，`fail_under=90` 通过；各 skill 覆盖率均 ≥ 93%（zentao 99%、jenkins_daily 99%、gerrit 100%、jenkins 99%、bug_daily 98-99%、mysql 95%、email 93%）
- **改动文件**：
  ...
  - `jenkins_daily_analysis/scripts/collect.py` — #2 list-builds 传 `--limit 0`（不限）
  - `jenkins_daily_analysis/scripts/report.py` — #3 非标准 category 归一化为 unknown
  - `jenkins_daily_analysis/scripts/fetch.py` — #13 缺 job/number 兜底跳过，不中断整批
  - `jenkins_daily_analysis/tests/test_fetch.py` / `test_report.py` — 补缺键 / 归一化用例
  - `bug_daily_analysis/scripts/bug_analysis.py` — #4 `--since`/`--until` 格式校验防 SQL 注入
  - `bug_daily_analysis/scripts/render_report.py` — #5 MD 表格单元格转义 `|` 与换行
  - `bug_daily_analysis/tests/test_bug_analysis.py` / `test_render_report.py` — 补校验 / 转义用例
